...
if __name__ == '__main__':
    ...
    basename = cfg['outdir']
    a_all = np.array([float(f.split('_a')[-1].split('_')[0]) for f in glob('{}/basis_spectra_nbody_pk_rsd={}_pypower={}_a*_nmesh*.npy'.format(basename, rsd, cfg['use_pypower'])) if len(f.split('_a')[-1].split('_')[0])==6])
    a_all.sort()
